...
for ind, data in enumerate(datas):
    ...
    for i in range(len(axs)):
        for _, row in dfs[i].iterrows():
            axs[i].scatter(row.pl//1000, math.log(row.total_cost), c=colors[bss.index(row.bs)], marker=markers[lrs.index(row.lr)], edgecolors=edgecolors[drs.index(row.dr)], s=80)
    ...
    fig.supxlabel('Power Limit')
    fig.supylabel('Log Cost')
    fig.legend(handles, [f"BS {bs}" for bs in bss] + [f"LR {lr}" for lr in lrs] + [f"DR {dr}" for dr in drs], ncol=3, loc='upper left', framealpha=1)
    # fig.tight_layout()
    fig.suptitle(f"Profiling results for {figNames[ind]}")
    plt.show()