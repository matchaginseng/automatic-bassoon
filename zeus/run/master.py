...
class ProfilePLMaster(ZeusMaster):
    ...
    def __init__(
        ...
        super().__init__(
            batch_size_optimizer=batch_size_optimizer, 
            log_base=log_base,
    ...
    def run_job(
        ...
        """
        # Generate job command
        command = job.gen_command(batch_size, learning_rate, power_limit, seed, rec_i)
        ...
        # Reporting
        print(f"[run job] Launching job with BS {batch_size}, PL {power_limit}:")
        print(f"[run job] {zeus_env=}")
        if job.workdir is not None:
            print(f"[run job] cwd={job.workdir}")
        print(f"[run job] {command=}")
