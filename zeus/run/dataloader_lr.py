...
class PowerOptimizerDataLoader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._is_train:
            if PowerOptimizerDataLoader.train_batch_size != 0:
            ...
            PowerOptimizerDataLoader.train_batch_size = self.batch_size * self.world_size
        ...
        self.train_json = (
            f"{self.logdir}/{self.job_id}+bs{self.train_batch_size}+lr{self.learning_rate:.5f}.train.json"
        )
        self.power_json = f"{self.logdir}/bs{self.train_batch_size}+lr{self.learning_rate:.5f}.power.json"
        ...
        if not self._is_train:
            PowerOptimizerDataLoader.eval_num_samples = self.num_samples
        ...
        if self._is_train:
            ...
            PowerOptimizerDataLoader.train_epoch_energy = np.zeros(
                shape=(self.world_size, self.max_epochs), dtype=np.float64
            )
            PowerOptimizerDataLoader.eval_epoch_energy = np.zeros(
                shape=(self.world_size, self.max_epochs), dtype=np.float64
            )

        # Whether profiling is done
    ...
    def save_metric(self, metric: float) -> None:
        with open(self.history_file_all, "w") as f:
            f.write(
                '''
                {
                    ...
                    accuracy: {accuracy}
                '''
                .format(bs=self.batch_size, pl=self.current_gpu_pl, lr=self.learning_rate, energy=self.sum_avg_power, time=self.time_consumed, accuracy=metric))
    ...
    def _compute_optimal_pl(self) -> int:
        ...
        # Sanity checks.
        assert PowerOptimizerDataLoader.train_tput_result
        assert PowerOptimizerDataLoader.train_power_result
        # Only compute optimal PL at master process.
        assert self.rank == 0

        # Compute power cost
        tput = PowerOptimizerDataLoader.train_tput_result
        power = PowerOptimizerDataLoader.train_power_result
        cost_map = {
            pl: (
    ...
    def _set_gpu_power_limit(self, power_limit: int) -> None:
        ...
        if self.current_gpu_pl != power_limit:
            ...
            PowerOptimizerDataLoader.current_gpu_pl = power_limit
    ...
    def _set_gpu_steady_power_limit(self) -> None:
        ...
        power_limit = PowerOptimizerDataLoader.optimal_pl if self.use_optimal_pl else self.max_pl
        self._log(
    ...
    def _start_monitor(self) -> None:
        ...
        # Sanity checks.
        assert not PowerOptimizerDataLoader.monitors
        ...
        if self.rank == 0:
            ...
            for index in range(self.world_size):
                ...
                try:
                    ...
                    raise
                self.sum_avg_power += avg_power
            self.train_power_result[self.current_gpu_pl] = self.sum_avg_power
    ...
    def _load_power_results(self) -> None:
        ...
        PowerOptimizerDataLoader.train_power_result = as_int_key(power_results["train_power"])
        PowerOptimizerDataLoader.train_tput_result = as_int_key(power_results["train_throughput"])
        PowerOptimizerDataLoader.eval_power_result = as_int_key(power_results["eval_power"])
        PowerOptimizerDataLoader.eval_tput_result = as_int_key(power_results["eval_throughput"])
        PowerOptimizerDataLoader.optimal_pl = power_results["optimal_pl"]
...
#                    raise
    ...
    def get_optimal_pl(self):
        return self.optimal_pl
    
    def get_energy(self):
        """Returns energy usage for optimal power limit."""
        return 0
